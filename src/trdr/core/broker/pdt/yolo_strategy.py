...
class YoloStrategy(BasePDTStrategy):
    ...
    def evaluate_order(self, context: PDTContext) -> PDTDecision:
        ...
        if context.side == OrderSide.BUY:
            ...
            return PDTDecision(
                allowed=True,
                reason="Order allowed: YOLO strategy permits unlimited buys"
            )
            ...
            if not context.position_opened_today:
                ...
                return PDTDecision(
                    allowed=True,
                    reason="Order allowed: position not opened today"
                )
            else:
                # Never allow same-day sells
                return PDTDecision(
                    allowed=False,
                    reason="Cannot sell position opened today under YOLO strategy"
                )
